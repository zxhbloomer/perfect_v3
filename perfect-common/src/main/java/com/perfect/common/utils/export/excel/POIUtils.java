...
public class POIUtils {
    ...
    static void writeByLocalOrBrowser(
        HttpServletResponse response, String fileName, SXSSFWorkbook wb, OutputStream out) {
        try {
            ZipSecureFile.setMinInflateRatio(0L);
            if (response != null) {
                // response对象不为空,响应到浏览器下载
                response.setContentType(PerfectConstant.XLSX_CONTENT_TYPE);
                response.setHeader("Content-disposition", "attachment; filename="
                        + URLEncoder.encode(String.format("%s%s", fileName, PerfectConstant.XLSX_SUFFIX), "UTF-8"));
                if (out == null) {
    ...
    private static void checkExcelFile(String file) {
        if (!file.endsWith(PerfectConstant.XLSX_SUFFIX)) {
